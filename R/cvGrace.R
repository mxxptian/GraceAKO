...
#' @param Y n by 1 vector of the response variable.
#' @param L  p by p symmetric matrix of the penalty weight matrix.
...
cvGrace <- function(X, Y, L, lambda.L, lambda.1, lambda.2, K = 10){
  ...
  for(iL in 1:length(lambda.L)){
    ...
    for(i2 in 1:length(lambda.2)){
      ...
      l1star <- lambda.1
      Xstar <- rbind(as.matrix(X), t(S)) / sqrt(2)
      Ystar <- c(Y, rep(0, p))
      gammastar <- l1star / sqrt(2) / 2 / (n + p)
      cvres <- cv.glmnet(Xstar, Ystar, lambda = gammastar, intercept = FALSE, standardize = FALSE, nfolds = 10)
